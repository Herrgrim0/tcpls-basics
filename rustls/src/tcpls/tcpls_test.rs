use crate::tcpls::constant::{MAX_RECORD_SIZE, STREAM_FRAME, STREAM_FRAME_FIN};
...
fn test_slice_to_u64_small_value() {
    ...
    assert_eq!(
        conversion::slice_to_u64(&slc_3).unwrap(),
        172516785778717348_u64
    );
}
...
fn test_stream_creation_max_frame_size() {
    ...
    assert_eq!(
        stream
            .create_stream_frame(MAX_RECORD_SIZE)
            .unwrap(),
        data
    );
}
